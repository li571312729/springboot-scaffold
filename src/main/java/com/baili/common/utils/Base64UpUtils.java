...
@Component
public class Base64UpUtils {

    public final static Map<String, String> FILE_TYPE_MAP = new HashMap<String, String>();

    private Base64UpUtils() {
    }
     ...
     * @return
     */
    public String Base64ToImage(String imgStr) throws IOException { // 对字节数组字符串进行Base64解码并生成图片
        if (StringUtils.isBlank(imgStr)) { // 图像数据为空
            throw new BaseException(1020, "图像数据为空");
        }
        ...
        StorePath storePath = fastFileStorageClient.uploadFile(ins, ins.available(), getFileTypeByStream(b), null);
